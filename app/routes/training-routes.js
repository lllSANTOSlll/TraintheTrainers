...
const { isAuthenticated, isSupervisorOrAdmin } = require('../middleware/auth');
...
router.get('/', (req, res) => {
  ...
  if (userRole === 'admin' || userRole === 'supervisor') {
    ...
    // Formateurs and Users only see sessions assigned to them
    sql = `SELECT ts.*, t.nom_prenom as trainer_name, u.username as assigned_username, u.full_name as assigned_fullname
           FROM training_sessions ts
           JOIN trainers t ON ts.trainer_id = t.id
           LEFT JOIN users u ON ts.assigned_user_id = u.id
...
// New training session form - SUPERVISOR/ADMIN ONLY
router.get('/new', isSupervisorOrAdmin, (req, res) => {
  ...
  // Show formateurs and users as assignable
...
// Create training session - SUPERVISOR/ADMIN ONLY
